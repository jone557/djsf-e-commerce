        ...
        if (cartItems) {
          dispatch({
            ...
            payload: {
              ...
              //   img: `http://localhost:5000/images/uploads/${cartItems[key].img}`,
...
export const addToCart = (product, newQty = 1) => {
  return async (dispatch) => {
    ...
    if (auth.isAuthenticated) {
      ...
      try {
        const res = await axios.post('http://localhost:5000/user/cart/addtocart', payload);
...
export const removeCartItem = (payload) => {
  return async (dispatch) => {
    try {
      ...
      const res = await axios.post('http://localhost:5000/user/cart/removecart', { payload });
      if (res.status === 202) {
