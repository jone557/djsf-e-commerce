...
const MyWishList = ({ product }) => {
    const useStyles = makeStyles((theme) => ({
        ...
        media: {
            height: '300px',
            position: 'relative',
            backgroundImage: `url(http://localhost:5000/images/uploads/${product.images[0]})`,
    ...
    const handleBgChange = (e) => {
        e.target.style.backgroundImage = `url(http://localhost:5000/images/uploads/${product.images[1]})`
    }
    const handleBgLeave = (e) => {
        e.target.style.backgroundImage = `url(http://localhost:5000/images/uploads/${product.images[0]})`
    }
    const classes = useStyles()
    return (
        <div className={classes.container} >
            <Link to={`/products/${product.id}`}>
                <div className={classnames(classes.media)} onMouseEnter={handleBgChange}
            ...
            <div className={classes.content}>
                <Link to={`/products/${product.id}`}>
                    ...
                    <Typography variant="body1" color="textSecondary" component="p">{product.description}
                    </Typography>
