...
const CustomCard = ({ classes, lipimage, liptitle, lipprice, hoverimage, lipdescription, id }) => {
      ...
      <Card className={classes.lipcard}>
        ...
        <Link to={`/products/${id}`}>
          <div style={{ maxWidth: '320px', height: "320px", }} >
            <img src={`http://localhost:5000/images/uploads/${lipimage}`} style={{ width: '100%', height: "320px", }}
              onMouseOver={e => (e.currentTarget.src = `http://localhost:5000/images/uploads/${hoverimage}`)}
              onMouseOut={e => (e.currentTarget.src = `http://localhost:5000/images/uploads/${lipimage}`)} objectFit='cover' alt="This is product" />
          ...
          <CardContent className={classes.lipcontent}>
            ...
            <Typography variant="body1" color="textSecondary" component="p" className={classes.liptitle} >
              {liptitle}
            </Typography>
            <Typography variant="body1" color="textSecondary" className={classes.lipdescription}>
              {lipdescription}
            </Typography>
            <Typography className={classes.lipprice} variant="body1" gutterBottom style={{ fontWeight: 'bold' }}>${lipprice}</Typography>
          </CardContent>
        </Link>
        <Typography className={classes.lipcontent} >
          <Button aria-label="add to cart" style={AddtoCart} onClick={() => {
            const _id = id;
...
const Lips = ({ name, price, image, image1, description, id }) => {
  ...
  return (
    <div >
      <Grid container spacing={4} xs={12} sm={6} md={4} wrap={'nowrap'} style = {{display: 'flex', alignItems: 'center', justifyContent: 'center', margin: 'auto'}}>
        <Grid item>
